...
type RuntimeLazyChunkName = "weekly" | "reference";
...
const JODI_CONTEXT_SUMMARY_PATH = "Jodi_Data/context_summary.json";
...
function runtimeBaseScript(bundle: DashboardBundle): string {
  ...
  return [
    ...
    `window.BALANCE_LAZY_FILES = ${scriptPayload({ weekly: lazyFiles.weekly, reference: lazyFiles.reference })};`,
    "window.__BALANCE_CHUNKS__ = window.__BALANCE_CHUNKS__ || {};",
    "",
  ].join("\n");
}

function runtimeChunkScript(chunkName: RuntimeLazyChunkName, payload: RuntimeWeeklyChunk | RuntimeReferenceChunk): string {
...
function regionPoint(period: string, status: "actual" | "forecast", regionKey: string, regionName: string, values: Omit<RegionalBalancePoint, "period" | "status" | "regionKey" | "regionName" | "stockChangeKbd" | "balanceKbd"> & { stockChangeKbd?: number }): RegionalBalancePoint {
  const balanceKbd = round(values.productionKbd + values.importsKbd + values.netReceiptsKbd - values.exportsKbd - values.demandKbd);
  return {
    period,
    status,
    regionKey,
    ...
    importsKbd: round(values.importsKbd),
...
function directMonthlyRegion(config: ProductConfig, row: CsvRow, period: string, area: "padd1" | "padd2" | "padd3" | "padd4" | "padd5" | "us", regionKey: string, regionName: string): RegionalBalancePoint {
  ...
  return regionPoint(period, "actual", regionKey, regionName, {
    ...
    importsKbd: isUs ? sumColumns(row, config.monthlyColumns.imports) : valueByColumn(row, importsCol),
...
function monthlyPadd1Splits(config: ProductConfig, row: CsvRow, period: string): [RegionalBalancePoint, RegionalBalancePoint] {
  ...
  return [
    regionPoint(period, "actual", "padd1ab", "P1-A/B Northeast", {
      ...
      importsKbd: p1.importsKbd * shareAB,
    ...
    regionPoint(period, "actual", "padd1c", "P1-C Lower Atlantic", {
      ...
      importsKbd: p1.importsKbd * shareC,
...
function monthlyRegionalBalances(config: ProductConfig, rows: CsvRow[]): RegionalBalancePoint[] {
  ...
  for (let period = addMonths(latest, 1); period <= MONTHLY_FORECAST_THROUGH; period = addMonths(period, 1)) {
    ...
    BASE_REGION_KEYS.forEach((key) => {
      ...
      const importsKbd = round(average(season.map((point) => point.importsKbd)) || previous?.importsKbd || 0);
...
type JodiProductContextSummary = {
  latestPeriod: string;
  regions: JodiContextSummaryRegion[];
  note: string;
};

type JodiContextSummary = {
  schemaVersion: number;
  generatedAt: string;
  products: Partial<Record<ProductKey, JodiProductContextSummary>>;
};

function jodiProductPaths(product: ProductKey): Array<{ region: string; path: string }> {
  const paths = [
    { region: "Europe", path: `Jodi_Data/europe_${product}.csv` },
    { region: "Africa", path: `Jodi_Data/africa_${product}.csv` },
  ];
  return paths;
}

function jodiContextSummaryFresh(): boolean {
  if (!fileExists(JODI_CONTEXT_SUMMARY_PATH)) return false;
  const summaryMtime = statSync(JODI_CONTEXT_SUMMARY_PATH).mtimeMs;
  return PRODUCTS.flatMap((product) => jodiProductPaths(product.key))
    .filter(({ path }) => fileExists(path))
    .every(({ path }) => statSync(path).mtimeMs <= summaryMtime);
}

function buildJodiProductContextSummary(product: ProductKey): JodiProductContextSummary {
  const latestPeriods: string[] = [];
  const regions = jodiProductPaths(product).flatMap(({ region, path }) => {
    const rows = rowsForOptionalCsv(path);
    ...
    let latestPeriod = "";
    for (const row of rows) {
      if (row.unit_measure_code !== "KBD" || !row.value || row.value_status === "-") continue;
      const period = row.period_month;
      if (!period) continue;
      latestPeriod = period > latestPeriod ? period : latestPeriod;
      ...
      const flow = row.flow_breakdown_code;
      if (flow === "TOTDEMO") bucket.demandKbd += num(row.value);
      else if (flow === "TOTIMPSB") bucket.importsKbd += num(row.value);
      else if (flow === "TOTEXPSB") bucket.exportsKbd += num(row.value);
      if (row.country_code) bucket.countries.add(row.country_code);
      if (row.assessment_code === "1") bucket.assessed += 1;
      bucket.latestRows += 1;
      byPeriod.set(period, bucket);
    }
    if (latestPeriod) latestPeriods.push(latestPeriod);
    const latest = byPeriod.get(latestPeriod);
    if (!latest) return [];
    return [{
      region,
      demandKbd: round(latest.demandKbd),
      importsKbd: round(latest.importsKbd),
      exportsKbd: round(latest.exportsKbd),
      countries: latest.countries.size,
      rows: rows.length,
      assessedShare: latest.latestRows ? round(latest.assessed / latest.latestRows * 100, 1) : 0,
    }];
  });
  const latestPeriod = latestPeriods.sort().at(-1) ?? "";
  return {
    latestPeriod,
    regions,
    note: `JODI ${product} regional context aggregates KBD demand, imports, and exports for Europe and Africa when local JODI files are present.`,
  };
}

function readJodiContextSummary(): JodiContextSummary {
  if (jodiContextSummaryFresh()) {
    try {
      return JSON.parse(readFileSync(JODI_CONTEXT_SUMMARY_PATH, "utf8")) as JodiContextSummary;
    } catch {
      // Rebuild below when the summary is unreadable.
    }
  }

  const summary: JodiContextSummary = {
    schemaVersion: 1,
    generatedAt: new Date().toISOString(),
    products: {
      diesel: buildJodiProductContextSummary("diesel"),
      jet: buildJodiProductContextSummary("jet"),
    },
  };
  mkdirSync(dirname(JODI_CONTEXT_SUMMARY_PATH), { recursive: true });
  writeFileSync(JODI_CONTEXT_SUMMARY_PATH, JSON.stringify(summary, null, 2) + "\n");
  return summary;
}

function buildJodiContext(config: ProductConfig): DashboardBundle["sourceHub"]["context"]["jodi"] {
  const product = config.key === "jet" ? "jet" : "diesel";
  const summary = readJodiContextSummary().products[product];
  ...
  return {
    ...
    latestPeriod,
    regions,
    note: summary?.note ?? `JODI ${config.key} regional context aggregates KBD demand, imports, and exports for Europe and Africa when local JODI files are present.`,
  };
}
...
function buildSourceHub(config: ProductConfig, bundleLike: {
  ...
  const jodi = buildJodiContext(config);
  const sources: DashboardBundle["sourceHub"]["sources"] = [
    ...
    {
      key: "jodi_world_database",
      label: "JODI international product context",
      category: "New source",
      status: jodi.available ? "active" : "candidate",
      latest: jodi.latestPeriod || "not packaged",
      coverage: "Europe and Africa demand/import/export context",
      rows: jodi.regions.reduce((sum, region) => sum + region.rows, 0),
      url: "https://www.jodidata.org/oil/database/overview.aspx",
      note: jodi.note,
    },
  ];
  return { sources, context: { powerDfo, jodi } };
}
...
<body>
  ...
  <script>
    ...
    function hasWeeklyData(){ return Array.isArray(D.regionalBalance?.weekly) && D.regionalBalance.weekly.length > 0 && Array.isArray(D.crudeRuns?.weekly) && D.crudeRuns.weekly.length > 0; }
    ...
    async function ensureDataForState(nextState=state){ if (nextState.frequency === 'weekly') await ensureWeeklyData(); if (nextState.sheet === 'reference' || nextState.sheet === 'outages' || nextState.sheet === 'crude') await ensureReferenceData(); }
    ...
    function buildChartScenarioDerivedAdjustments(){ const scenarios = activeChartScenarios().filter(scenario => scenario.metricKey !== 'exPlannedUtilization'); if (!scenarios.length) return []; const key = stableSettingsRows({forecastEnd:workbookSettings?.forecastEnd,adjustments:workbookSettings?.adjustments,scenarios}); if (chartScenarioDerivedAdjustmentsCache.key === key && chartScenarioDerivedAdjustmentsCache.rows) return chartScenarioDerivedAdjustmentsCache.rows; const index = new Map(); const setAdjustment = adjustment => { if (!adjustment) return; index.set(balanceAdjustmentIndexKey(adjustment.frequency, adjustment.period, adjustment.regionKey, adjustment.lineId), adjustment); }; scenarios.forEach(scenario => { const metric = chartScenarioMetricByKey(scenario.metricKey); const periods = chartScenarioPeriods(scenario.startPeriod, scenario.endPeriod).filter(period => period >= firstForecastMonth() && period <= workbookSettings.forecastEnd.slice(0,7)); periods.forEach(periodMonth => { if (scenario.metricKey === 'receipts') { movementRowsForRegion(scenario.regionKey, 'in').forEach(flow => { const monthlyRow = movementRow('monthly', periodMonth, flow.id); if (monthlyRow?.status === 'forecast') { const baseMonthly = index.get(balanceAdjustmentIndexKey('monthly', periodMonth, scenario.regionKey, receiptLineId(flow.id)))?.valueKbd ?? scenarioExistingAdjustmentValue('monthly', periodMonth, scenario.regionKey, receiptLineId(flow.id)) ?? movementBaseValue('monthly', periodMonth, flow.id); if (Number.isFinite(baseMonthly)) setAdjustment(normalizeBalanceAdjustment({frequency:'monthly',period:periodMonth,regionKey:scenario.regionKey,lineId:receiptLineId(flow.id),valueKbd:roundScenarioMetricValue('receipts', chartScenarioAdjustedValue('receipts', baseMonthly, scenario.deltaPct)),note:'Chart scenario: ' + scenario.name,updatedAt:scenario.updatedAt})); } scenarioBaseWeeklyPeriods(periodMonth).forEach(period => { const weeklyRow = movementRow('weekly', period, flow.id); if (weeklyRow?.status !== 'forecast') return; const baseWeekly = index.get(balanceAdjustmentIndexKey('weekly', period, scenario.regionKey, receiptLineId(flow.id)))?.valueKbd ?? scenarioExistingAdjustmentValue('weekly', period, scenario.regionKey, receiptLineId(flow.id)) ?? movementBaseValue('weekly', period, flow.id); if (!Number.isFinite(baseWeekly)) return; setAdjustment(normalizeBalanceAdjustment({frequency:'weekly',period,regionKey:scenario.regionKey,lineId:receiptLineId(flow.id),valueKbd:roundScenarioMetricValue('receipts', chartScenarioAdjustedValue('receipts', baseWeekly, scenario.deltaPct)),note:'Chart scenario: ' + scenario.name,updatedAt:scenario.updatedAt})); }); }); return; } const point = scenarioBaseMonthlyPoint(scenario.regionKey, periodMonth); if (!point || point.status !== 'forecast') return; const baseValue = index.get(balanceAdjustmentIndexKey('monthly', periodMonth, scenario.regionKey, metric.lineId))?.valueKbd ?? scenarioExistingAdjustmentValue('monthly', periodMonth, scenario.regionKey, metric.lineId) ?? scenarioPointValue(point, scenario.metricKey); if (!Number.isFinite(baseValue)) return; setAdjustment(normalizeBalanceAdjustment({frequency:'monthly',period:periodMonth,regionKey:scenario.regionKey,lineId:metric.lineId,valueKbd:roundScenarioMetricValue(scenario.metricKey, chartScenarioAdjustedValue(scenario.metricKey, baseValue, scenario.deltaPct)),note:'Chart scenario: ' + scenario.name,updatedAt:scenario.updatedAt})); }); }); const rows = Array.from(index.values()).sort((a,b)=>a.frequency.localeCompare(b.frequency) || a.period.localeCompare(b.period) || a.regionKey.localeCompare(b.regionKey) || a.lineId.localeCompare(b.lineId)); chartScenarioDerivedAdjustmentsCache = {key,rows}; return rows; }
    ...
    async function saveBalanceAdjustmentsToServer(){ saveBalanceAdjustmentsLocal(); try { const response = await fetch(updateBaseUrl() + '/api/settings', {method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(settingsPayload())}); if (!response.ok) throw new Error('save '+response.status); const payload = await response.json(); const rows = payload.settings?.adjustments?.[D.product.key]; if (Array.isArray(rows)) { const before = stableSettingsRows(workbookSettings.adjustments); const next = normalizeBalanceAdjustmentList(rows); workbookSettings.adjustments = next; if (before !== stableSettingsRows(next)) { invalidateCalculationCaches(); queueRender(); } saveWorkbookSettingsLocal(); } showToast('Balance adjustments saved'); } catch { showToast('Balance adjustments saved in this browser; local runner offline'); } }
    ...
    function balanceLines(regionKey){ const baseRegion = isBaseBalanceRegion(regionKey); const receipts = receiptLines(regionKey); const receiptRows = receiptDisplayLines(regionKey); const p3Receipts = receipts.filter(line => flowById(lineFlowId(line.id))?.fromRegionKey === 'padd3'); const shipments = shipmentLines(regionKey); return [{id:'supplySection',label:'Supply',kind:'section'},...productionBalanceLines(regionKey),{id:'imports',label:'Total Imports',kind:'anchor'},...(baseRegion ? [{id:'importsAdjustment',label:'Imports Adjustment',kind:'item muted adjustment'}] : []),{id:'receiptsIn',label:'Total Receipts',kind:receipts.length ? 'subtotal anchor' : 'item muted'},...(p3Receipts.length ? [{id:'receiptsP3',label:'Total P3 Receipts',kind:'subtotal anchor'}] : []),...receiptRows,{id:'totalSupply',label:'Total supply',kind:'subtotal'},{id:'supplyDemandDivider',label:'',kind:'divider'},{id:'demandSection',label:'Demand',kind:'section'},{id:'demand',label:'Product supplied',kind:'item'},...(baseRegion ? [{id:'demandAdjustment',label:'Product Supplied Adjustment',kind:'item muted adjustment'}] : []),{id:'exports',label:'Exports',kind:'item'},...(baseRegion ? [{id:'exportsAdjustment',label:'Exports Adjustment',kind:'item muted adjustment'}] : []),{id:'receiptsOut',label:'Shipments to other PADDs',kind:shipments.length ? 'subtotal' : 'item muted'},...shipments,{id:'totalDemand',label:'Total demand',kind:'subtotal'},{id:'buildDaily',label:'Build/(draw) per day',kind:'highlight draw'},weeklyGuideLine('buildTotal'),{id:'buildTotal',label:'Total period build/(draw)',kind:'highlight draw'},{id:'stocks',label:'Ending stocks',kind:'stock'}]; }
    ...
    function valueForLine(point, lineId, regionKey=point?.regionKey){
      ...
      else if (lineId === 'imports') value = point.importsKbd;
    ...
    function rowClassForLine(line, values){ const parts = []; if (line.kind === 'section') parts.push('sectionRow'); if (line.kind.includes('subtotal')) parts.push('subtotalRow'); if (line.kind === 'divider') parts.push('dividerRow'); if (line.kind.includes('highlight')) parts.push('highlightRow'); if (line.kind.includes('muted')) parts.push('mutedRow'); if (line.kind.includes('adjustment')) parts.push('adjustmentRow'); if (line.kind === 'stock') parts.push('stockRow'); if (line.kind.includes('anchor')) parts.push('anchorSubtotalRow'); if (line.kind.includes('guide')) parts.push('guideRow'); if (isDemandZoneLine(line.id)) parts.push(line.kind === 'section' ? 'demandSectionBand' : line.kind.includes('subtotal') ? 'demandSubtotalBand' : 'demandZoneRow'); if (line.kind.includes('draw')) parts.push('drawRow', values.some(v => v < 0) ? 'negative' : 'positive'); return parts.join(' '); }
    ...
    function normalizedDraftForRender(){ const minMonth = firstForecastMonth(); const maxMonth = workbookSettings.forecastEnd.slice(0,7); const draft = {...(normalizeChartScenario(chartScenarioDraft, {previewOnly:true}) || defaultChartScenarioDraft())}; if (draft.startPeriod < minMonth) draft.startPeriod = minMonth; if (draft.endPeriod < minMonth) draft.endPeriod = minMonth; if (draft.startPeriod > maxMonth) draft.startPeriod = maxMonth; if (draft.endPeriod > maxMonth) draft.endPeriod = maxMonth; if (draft.endPeriod < draft.startPeriod) draft.endPeriod = draft.startPeriod; return draft; }
    ...
    function openChartScenarioModal(chartMetricKey, options={}){ const metricKey = chartScenarioMetricFromChartMetric(chartMetricKey || options.metricKey); if (!metricKey) return showToast('Forecast sliders are available on Demand, Imports, Exports, Receipts, and Yield charts'); const regionKey = chartScenarioRegionOptions().some(region => region.key === options.regionKey) ? options.regionKey : preferredChartScenarioRegion(); const startPeriod = firstForecastMonth(); const forecastEndMonth = workbookSettings.forecastEnd.slice(0,7); const endPeriod = normalizeScenarioMonth(options.endPeriod || forecastEndMonth, forecastEndMonth); chartScenarioDraft = defaultChartScenarioDraft({regionKey,metricKey,startPeriod,endPeriod:endPeriod < startPeriod ? startPeriod : endPeriod,name:String(options.name || chartScenarioDefaultName(regionKey, metricKey)).slice(0,48),deltaPct:Number.isFinite(Number(options.deltaPct)) ? clampScenarioDelta(options.deltaPct, metricKey) : 0,previewEnabled:false,updatedAt:new Date().toISOString()}); saveChartScenarioDraftLocal(); chartScenarioModalOpen = true; renderChartScenarioWorkbench(); }
    function renderChartScenarioWorkbench(){ const panel = document.getElementById('chartScenarioWorkbench'); if (!panel) return; if (state.sheet !== 'charts' || !chartScenarioModalOpen) { panel.hidden = true; panel.innerHTML = ''; return; } const draft = normalizedDraftForRender(); const scenarios = chartScenarios; const overlays = enabledChartScenarioOverlays(); const metric = chartScenarioMetricByKey(draft.metricKey); const draftMetricKey = chartMetricFromScenarioMetric(draft.metricKey); const range = chartScenarioRange(draft.metricKey); const overlayNames = overlays.map(scenario => scenario.name).join(', '); const summaryCard = (label, value, note, delta) => '<div class="scenarioSummaryCard '+(Number(delta || 0) > 0 ? 'positive' : Number(delta || 0) < 0 ? 'negative' : '')+'"><span>'+esc(label)+'</span><b>'+esc(value)+'</b><i>'+esc(note)+'</i></div>'; const usStocksDelta = chartScenarioLatestDelta('us', 'stocksKb', 'monthly'); const regionMetricDelta = chartScenarioLatestDelta(draft.regionKey, draftMetricKey, 'monthly'); panel.hidden = false; panel.innerHTML = '<div class="scenarioModalPanel"><div class="scenarioModalHeader"><div><div class="caption">Forecast workbench</div><h3>'+esc(metric.label)+' forecast</h3><p>Preview one live draft, save up to '+CHART_SCENARIO_LIMIT+' named forecast versions, and overlay any enabled saved forecasts directly on the chart canvas.</p></div><button class="btn" id="closeChartScenarioModalBtn" type="button">Close</button></div><div class="scenarioWorkbenchGrid"><div><div class="scenarioSummaryGrid">'+summaryCard('Saved forecasts', fmt(scenarios.length)+' / '+fmt(CHART_SCENARIO_LIMIT), overlays.length ? overlayNames : 'No overlays enabled', scenarios.length)+summaryCard('Enabled overlays', fmt(overlays.length), overlays.length ? overlayNames : 'Base chart only', overlays.length)+summaryCard('U.S. inventory delta', usStocksDelta === null ? 'n/a' : (usStocksDelta >= 0 ? '+' : '') + fmt(usStocksDelta), 'Latest monthly preview vs base', usStocksDelta)+summaryCard(balanceRegionDisplayLabel(draft.regionKey)+' '+metric.label+' delta', regionMetricDelta === null ? 'n/a' : (regionMetricDelta >= 0 ? '+' : '') + fmt(regionMetricDelta, metric.key === 'yield' ? 1 : 0), 'Latest monthly preview vs base', regionMetricDelta)+'</div><div class="scenarioDraftGrid"><label class="wide"><span class="small">Forecast name</span><input class="settingsInput" id="chartScenarioName" type="text" maxlength="48" value="'+esc(draft.name === 'Forecast' ? chartScenarioDefaultName(draft.regionKey, draft.metricKey) : draft.name)+'" placeholder="P1 imports forecast"></label><label><span class="small">Target region</span><select class="select" id="chartScenarioRegion">'+chartScenarioRegionOptions().map(region => '<option value="'+esc(region.key)+'" '+(region.key === draft.regionKey ? 'selected' : '')+'>'+esc(balanceRegionDisplayLabel(region.key))+'</option>').join('')+'</select></label><label><span class="small">Metric</span><div class="scenarioReadOnly">'+esc(metric.label)+(chartScenarioUsesRawVolume(metric.key) ? ' (kbd delta)' : ' (% delta)')+'</div></label><label><span class="small">From month</span><div class="scenarioReadOnly">'+esc(draft.startPeriod)+'</div></label><label><span class="small">To month</span><div class="scenarioFieldInline"><input class="settingsInput" id="chartScenarioEnd" type="month" min="'+esc(draft.startPeriod)+'" max="'+esc(workbookSettings.forecastEnd.slice(0,7))+'" value="'+esc(draft.endPeriod)+'"><button class="btn" id="chartScenarioEndPickerBtn" type="button">Calendar</button></div></label><label class="wide"><span class="small">Adjustment</span><div class="scenarioRangeRow"><input id="chartScenarioDelta" type="range" min="'+esc(range.min)+'" max="'+esc(range.max)+'" step="'+esc(range.step)+'" value="'+esc(draft.deltaPct)+'"><output id="chartScenarioDeltaLabel">'+esc(chartScenarioDeltaLabel(draft.metricKey, draft.deltaPct))+'</output></div></label></div><div class="scenarioActions"><button class="btn primary" id="previewChartScenarioBtn" type="button">'+(draft.previewEnabled ? 'Update preview' : 'Preview forecast')+'</button><button class="btn" id="saveChartScenarioBtn" type="button">Save forecast</button><button class="btn" id="clearChartScenarioBtn" type="button">'+(draft.previewEnabled ? 'Clear preview' : 'Reset draft')+'</button><span class="scenarioDraftPill">'+(draft.previewEnabled ? 'Preview active' : 'Draft only')+'</span></div><div class="scenarioStatus">'+esc(chartScenarioMetricRoundingText(draft.metricKey))+'</div><div class="scenarioHint">The To month field defaults to the active forecast end month, and saved forecasts stay local so they can be compared without overwriting the shared workbook.</div></div><div><div class="caption">Saved forecasts</div><div class="scenarioList">'+(scenarios.length ? scenarios.map(scenario => '<article class="scenarioRow"><div class="scenarioRowTop"><div><strong>'+esc(scenario.name)+'</strong><span>'+esc(balanceRegionDisplayLabel(scenario.regionKey))+' | '+esc(chartScenarioMetricByKey(scenario.metricKey).label)+'</span></div><label class="scenarioCheck"><input data-scenario-toggle="'+esc(scenario.id)+'" type="checkbox" '+(scenario.enabled ? 'checked' : '')+'>Overlay</label></div><div class="scenarioRowMeta"><span class="scenarioMetaPill">'+esc(scenario.startPeriod)+' to '+esc(scenario.endPeriod)+'</span><span class="scenarioMetaPill">'+esc(chartScenarioDeltaLabel(scenario.metricKey, scenario.deltaPct))+'</span><span class="scenarioMetaPill">saved '+esc(new Date(scenario.updatedAt).toLocaleString())+'</span></div><div class="scenarioRowActions"><button class="btn" data-scenario-load="'+esc(scenario.id)+'" type="button">Load</button><button class="btn" data-scenario-delete="'+esc(scenario.id)+'" type="button">Delete</button></div></article>').join('') : '<div class="scenarioListEmpty">No saved forecasts yet.</div>')+'</div><div class="scenarioScopeNote">Enable multiple saved forecasts to overlay them together on the matching chart.</div></div></div></div>'; }
    ...
    function renderChartScenarioWorkbench(){
      ...
      const scenarioRowsHtml = scenarios.length ? scenarios.map(scenario => '<article class="scenarioRow"><div class="scenarioRowTop"><div><strong>'+esc(scenario.name)+'</strong><span>'+esc(balanceRegionDisplayLabel(scenario.regionKey))+' | '+esc(chartScenarioMetricByKey(scenario.metricKey).label)+'</span></div><label class="scenarioCheck"><input data-scenario-toggle="'+esc(scenario.id)+'" type="checkbox" '+(scenario.enabled ? 'checked' : '')+'>Overlay</label></div><div class="scenarioRowMeta"><span class="scenarioMetaPill">'+esc(scenario.startPeriod)+' to '+esc(scenario.endPeriod)+'</span><span class="scenarioMetaPill">'+esc(chartScenarioDeltaLabel(scenario.metricKey, scenario.deltaPct))+'</span><span class="scenarioMetaPill">saved '+esc(new Date(scenario.updatedAt).toLocaleString())+'</span></div><div class="scenarioRowActions"><button class="btn" data-scenario-load="'+esc(scenario.id)+'" type="button">Load</button><button class="btn" data-scenario-delete="'+esc(scenario.id)+'" type="button">Delete</button></div></article>').join('') : '<div class="scenarioListEmpty">No saved forecasts yet.</div>';
      panel.hidden = false;
      panel.innerHTML = '<div class="scenarioModalPanel"><div class="scenarioModalHeader"><div><div class="caption">Forecast workbench</div><h3>'+esc(metric.label)+' forecast</h3><p>Preview one live draft, save up to '+CHART_SCENARIO_LIMIT+' named forecast versions, and overlay any enabled saved forecasts directly on the chart canvas.</p></div><button class="btn" id="closeChartScenarioModalBtn" type="button">Close</button></div><div class="scenarioWorkbenchGrid"><div><div class="scenarioSummaryGrid">'+summaryCard('Saved forecasts', fmt(scenarios.length)+' / '+fmt(CHART_SCENARIO_LIMIT), overlays.length ? overlayNames : 'No overlays enabled', scenarios.length)+summaryCard('Enabled overlays', fmt(overlays.length), overlays.length ? overlayNames : 'Base chart only', overlays.length)+summaryCard('U.S. inventory delta', usStocksDelta === null ? 'n/a' : (usStocksDelta >= 0 ? '+' : '') + fmt(usStocksDelta), 'Latest monthly preview vs base', usStocksDelta)+summaryCard(balanceRegionDisplayLabel(draft.regionKey)+' '+metric.label+' delta', regionMetricDelta === null ? 'n/a' : (regionMetricDelta >= 0 ? '+' : '') + fmt(regionMetricDelta, metric.key === 'yield' ? 1 : 0), 'Latest monthly preview vs base', regionMetricDelta)+'</div><div class="scenarioImpactGrid" id="chartScenarioImpactGrid">'+chartScenarioImpactHtml(draft)+'</div><div class="scenarioDraftGrid"><label class="wide"><span class="small">Forecast name</span><input class="settingsInput" id="chartScenarioName" type="text" maxlength="48" value="'+esc(draft.name === 'Forecast' ? chartScenarioDefaultName(draft.regionKey, draft.metricKey) : draft.name)+'" placeholder="P1 imports forecast"></label><label><span class="small">Target region</span><select class="select" id="chartScenarioRegion">'+chartScenarioRegionOptions().map(region => '<option value="'+esc(region.key)+'" '+(region.key === draft.regionKey ? 'selected' : '')+'>'+esc(balanceRegionDisplayLabel(region.key))+'</option>').join('')+'</select></label><label><span class="small">Metric</span><div class="scenarioReadOnly">'+esc(metric.label)+(chartScenarioUsesRawVolume(metric.key) ? ' (kbd delta)' : ' (% delta)')+'</div></label><label><span class="small">From month</span><div class="scenarioReadOnly">'+esc(draft.startPeriod)+'</div></label><label><span class="small">To month</span><div class="scenarioFieldInline"><input class="settingsInput" id="chartScenarioEnd" type="month" min="'+esc(draft.startPeriod)+'" max="'+esc(workbookSettings.forecastEnd.slice(0,7))+'" value="'+esc(draft.endPeriod)+'"><button class="btn" id="chartScenarioEndPickerBtn" type="button">Calendar</button></div></label><label class="wide"><span class="small">Adjustment</span><div class="scenarioRangeRow"><input id="chartScenarioDelta" type="range" min="'+esc(range.min)+'" max="'+esc(range.max)+'" step="'+esc(range.step)+'" value="'+esc(draft.deltaPct)+'"><output id="chartScenarioDeltaLabel">'+esc(chartScenarioDeltaLabel(draft.metricKey, draft.deltaPct))+'</output></div><div class="scenarioPresetRow" id="chartScenarioPresetRow">'+chartScenarioPresetHtml(draft)+'</div></label></div><div class="scenarioActions"><button class="btn primary" id="previewChartScenarioBtn" type="button">'+(draft.previewEnabled ? 'Update preview' : 'Preview forecast')+'</button><button class="btn" id="saveChartScenarioBtn" type="button">Save forecast</button><button class="btn" id="clearChartScenarioBtn" type="button">'+(draft.previewEnabled ? 'Clear preview' : 'Reset draft')+'</button><span class="scenarioDraftPill">'+(draft.previewEnabled ? 'Preview active' : 'Draft only')+'</span></div><div class="scenarioStatus">'+esc(chartScenarioMetricRoundingText(draft.metricKey))+'</div><div class="scenarioHint">Saved forecasts stay local so they can be compared without overwriting the shared workbook.</div></div><div><div class="caption">Saved forecasts</div><div class="scenarioList">'+scenarioRowsHtml+'</div><div class="scenarioScopeNote">Enable multiple saved forecasts to overlay them together on the matching chart.</div></div></div></div>';
    }
    ...
    function previewChartScenario(){ const draft = normalizedDraftForRender(); if (!draft.regionKey || !draft.metricKey) return showToast('Choose a region and metric'); if (!Number.isFinite(Number(draft.deltaPct))) return showToast('Choose an adjustment'); chartScenarioDraft = {...draft, previewEnabled:true, updatedAt:new Date().toISOString()}; saveChartScenarioDraftLocal(); renderChartScenarioWorkbench(); queueRender(); showToast('Forecast preview applied'); }
    function clearChartScenarioDraft(resetForm=false){ const draft = normalizedDraftForRender(); chartScenarioDraft = resetForm ? defaultChartScenarioDraft({regionKey:draft.regionKey,metricKey:draft.metricKey,startPeriod:firstForecastMonth(),endPeriod:workbookSettings.forecastEnd.slice(0,7),name:chartScenarioDefaultName(draft.regionKey, draft.metricKey)}) : {...draft, previewEnabled:false, updatedAt:new Date().toISOString()}; saveChartScenarioDraftLocal(); renderChartScenarioWorkbench(); queueRender(); showToast(resetForm ? 'Forecast draft reset' : 'Forecast preview cleared'); }
    function saveChartScenario(){ const draft = normalizedDraftForRender(); const delta = Number(draft.deltaPct || 0); if (!delta) return showToast('Move the slider before saving'); if (chartScenarios.length >= CHART_SCENARIO_LIMIT) return showToast('Save limit reached: keep at most ' + CHART_SCENARIO_LIMIT + ' forecasts'); const scenario = {...draft,id:'scenario-' + Date.now(),name:(draft.name || chartScenarioDefaultName(draft.regionKey, draft.metricKey)).slice(0,48),enabled:true,previewEnabled:false,updatedAt:new Date().toISOString()}; chartScenarios = normalizeChartScenarioList([...chartScenarios, scenario]); saveChartScenariosLocal(); chartScenarioDraft = defaultChartScenarioDraft({regionKey:draft.regionKey,metricKey:draft.metricKey,startPeriod:firstForecastMonth(),endPeriod:workbookSettings.forecastEnd.slice(0,7),name:chartScenarioDefaultName(draft.regionKey, draft.metricKey)}); saveChartScenarioDraftLocal(); chartScenarioModalOpen = false; renderChartScenarioWorkbench(); queueRender(); showToast('Forecast saved locally'); }
    ...
    function heatCellParts(regionKey, line, period, point, value){
      ...
      const lineId = isWeeklyGuideLine(line.id) ? weeklyGuideTargetLineId(line.id) : line.id;
      if (lineId !== 'buildDaily' && lineId !== 'buildTotal') return {classes:[],style:''};
      const perDay = lineId === 'buildTotal' ? numeric / Math.max(1, periodDays(period)) : numeric;
      const demand = Math.max(1, Math.abs(Number(totals(point).demand || point.demandKbd || 0)));
      const heat = Math.min(1, Math.abs(perDay) / Math.max(25, demand * .05));
      return {classes:['heatCell', perDay >= 0 ? 'heatPos' : 'heatNeg'],style:'--heat:'+heat.toFixed(3)};
    }
    ...
    function chartScenarioOverlaySeries(regionKey, metricKey, frequency=state.frequency){
      ...
      if (chartScenarioOverlaySeriesCache.has(overlaysKey)) return chartScenarioOverlaySeriesCache.get(overlaysKey);
      const activeOverlays = overlays.filter(scenario => scenario.metricKey === scenarioMetricKey && scenarioAffectsChartRegion(scenario, regionKey));
      ...
      const {currentYear, nextYearForecast} = chartYearContext(frequency);
      const baseRows = withChartScenariosSuppressed(() => rowsForRegion(regionKey, frequency));
      const baseBundle = chartSeriesBundle(baseRows, metricKey, frequency, {regionKey,cacheScope:'overlay-base:' + regionKey + ':' + metricKey});
      const baseActual = state.showCurrentYear ? chartBundleYearPoints(baseBundle, currentYear, 'actual') : [];
      const sourceValues = baseBundle.rawValues || baseBundle.values;
      const sourceValid = baseBundle.rawValid || baseBundle.valid;
      const series = activeOverlays.map((scenario, index) => {
        const scenarioRowByPeriod = chartScenarioRowsByPeriod(scenario.regionKey, frequency);
        const rawValues = new Float32Array(baseBundle.length);
        const rawValid = new Uint8Array(baseBundle.length);
        for (let rowIndex = 0; rowIndex < baseBundle.length; rowIndex += 1) {
          if (!sourceValid[rowIndex]) continue;
          let value = sourceValues[rowIndex];
          if (baseBundle.statuses[rowIndex] === 'forecast') {
            if (scenario.metricKey === 'yield' || scenario.metricKey === 'exPlannedUtilization') {
              if (chartScenarioTouchesPeriod(scenario, baseBundle.periods[rowIndex], frequency)) value = chartScenarioAdjustedValue(scenario.metricKey, value, scenario.deltaPct);
            } else {
              const delta = chartScenarioPointDelta(scenarioRowByPeriod.get(baseBundle.periods[rowIndex]), scenario, frequency);
              if (Number.isFinite(delta)) value = round2(value + delta);
            }
          }
          rawValues[rowIndex] = value;
          rawValid[rowIndex] = 1;
        }
        const adjusted = baseBundle.rolling ? rollingMean4(rawValues, rawValid) : {values:rawValues,valid:rawValid};
        const currentForecast = state.showCurrentYear ? collectOverlayForecastPoints(baseBundle, adjusted.values, adjusted.valid, currentYear) : [];
        const currentBridge = baseActual.length && currentForecast.length ? [baseActual.at(-1), ...currentForecast] : currentForecast;
        const nextForecast = state.showNextYearForecast && nextYearForecast ? collectOverlayForecastPoints(baseBundle, adjusted.values, adjusted.valid, nextYearForecast) : [];
        return {id:scenario.id,name:scenario.name,color:CHART_SCENARIO_OVERLAY_PALETTE[index % CHART_SCENARIO_OVERLAY_PALETTE.length],currentBridge,nextForecast};
      }).filter(seriesRow => seriesRow.currentBridge.length || seriesRow.nextForecast.length);
      chartScenarioOverlaySeriesCache.set(overlaysKey, series);
      return series;
    ...
    function crudeLines(regionKey){ const lines = [{id:'capacitySection',label:'Capacity',kind:'section'},{id:'operableCapacityKbd',label:'Operable Capacity',kind:'item operating'},{id:'idleCapacityKbd',label:'Idle Capacity',kind:'item muted'}]; if (validBaseCrudeRegion(regionKey)) lines.push({id:'idleCapacityAdjustmentKbd',label:'Idle Capacity Adjustment',kind:'item muted adjustment'}); lines.push({id:'operatingCapacityKbd',label:'Operating Capacity',kind:'subtotal'},{id:'plannedMaintenanceKbd',label:'Planned Atmos Offline',kind:'item muted maintenance'},{id:'unplannedMaintenanceKbd',label:'Unplanned Atmos Offline',kind:'item muted maintenance'},{id:'totalOfflineKbd',label:'Total Offline',kind:'subtotal'},{id:'runsSection',label:'Runs',kind:'section'},{id:'crudeRunsKbd',label:'Crude Runs',kind:'item'},{id:'operatingUtilizationPct',label:'Operating Utilization',kind:'highlight'},{id:'exPlannedUtilizationPct',label:'Operating (ex-planned) Utilization',kind:'highlight'}); if (validBaseCrudeRegion(regionKey)) lines.push({id:'exPlannedUtilizationAdjustmentPct',label:'Operating (ex-planned) Utilization Adjustment',kind:'item muted adjustment percent'}); return lines; }
    ...
    function crudeLineDisplayLabel(line){ return isCrudeLineAdjustable(line.id) ? 'Override' : line.label; }
    ...
    function queueBalanceAdjustmentsToServer(){ clearTimeout(balanceAdjustmentSaveTimer); balanceAdjustmentSaveTimer = setTimeout(() => { balanceAdjustmentSaveTimer = 0; saveBalanceAdjustmentsToServer(); }, 650); }
    ...
    function renderContextPulse(){ const hub = D.sourceHub || {sources:[],context:{}}; const power = hub.context?.powerDfo || {}; const jodi = hub.context?.jodi || {}; const powerAvailable = Boolean(power.available); const jodiAvailable = Boolean(jodi.available); const jodiTotalDemand = (jodi.regions || []).reduce((sum, row) => sum + Number(row.demandKbd || 0), 0); const activeSources = (hub.sources || []).filter(source => ['active','packaged'].includes(source.status)).length; const sourceBadges = (hub.sources || []).map(source => '<span class="sourceBadge">'+sourceStatusDot(source.status)+esc(source.label)+'</span>').join(''); const powerNote = powerAvailable ? 'PADD 1 city basket' : (power.note || 'not available'); document.getElementById('referencePulse').innerHTML = '<div class="panel pad"><div class="caption">External context</div><div class="signalGrid"><div class="signalCard"><span>Power DFO latest</span><b>'+(powerAvailable ? fmt(power.latestKbd,1)+' kb/d' : 'n/a')+'</b><i>'+esc(powerAvailable ? power.latestDate : powerNote)+'</i></div><div class="signalCard"><span>Power MTD burn</span><b>'+(powerAvailable ? fmt(power.monthToDateKb,1)+' kb' : 'n/a')+'</b><i>'+(powerAvailable ? fmt(power.monthAverageKbd,1)+' kb/d avg' : esc(powerNote))+'</i></div><div class="signalCard"><span>Next 24h burn</span><b>'+(powerAvailable ? fmt(power.forecast24hKb,1)+' kb' : 'n/a')+'</b><i>'+(powerAvailable ? 'peak '+fmt(power.forecastPeakKbd,1)+' kb/h' : esc(powerNote))+'</i></div><div class="signalCard"><span>JODI demand</span><b>'+(jodiAvailable ? fmt(jodiTotalDemand)+' kb/d' : 'n/a')+'</b><i>'+esc(jodiAvailable ? jodi.latestPeriod : 'not available')+'</i></div></div></div><div class="panel pad"><div class="caption">Input inventory</div><div class="signalGrid"><div class="signalCard"><span>Active sources</span><b>'+activeSources+'/'+(hub.sources || []).length+'</b><i>active or packaged</i></div><div class="signalCard"><span>Weather temp</span><b>'+(powerAvailable ? fmt(power.avgTempF,1)+' F' : 'n/a')+'</b><i>'+esc(powerNote)+'</i></div></div><div class="sourceRibbon" style="margin-top:9px">'+sourceBadges+'</div></div>'; }
    function renderSourceSheet(){ const hub = D.sourceHub || {sources:[],context:{jodi:{regions:[]}}}; document.getElementById('sourceCount').textContent = (hub.sources || []).length + ' sources'; const sourceCards = (hub.sources || []).map(source => '<article class="sourceCard"><h3>'+esc(source.label)+'</h3><div class="sourceMeta"><span class="sourceStatus '+esc(source.status)+'">'+esc(statusLabel(source.status))+'</span><span class="sourceStatus">'+esc(source.category)+'</span></div><dl><dt>Latest</dt><dd>'+esc(source.latest || 'n/a')+'</dd><dt>Coverage</dt><dd>'+esc(source.coverage || 'n/a')+'</dd><dt>Rows</dt><dd>'+fmt(source.rows || 0)+'</dd><dt>Source</dt><dd><a class="sourceLink" href="'+esc(source.url)+'" target="_blank" rel="noopener">'+esc(source.url)+'</a></dd></dl><p>'+esc(source.note || '')+'</p></article>').join(''); const jodiRows = ((hub.context?.jodi?.regions) || []).map(row => '<tr><td>'+esc(row.region)+'</td><td>'+fmt(row.demandKbd)+'</td><td>'+fmt(row.importsKbd)+'</td><td>'+fmt(row.exportsKbd)+'</td><td>'+fmt(row.countries)+'</td><td>'+fmt(row.assessedShare,1)+'%</td></tr>').join(''); const jodiTable = '<div class="sourceCard"><h3>JODI international '+esc(D.product.shortTitle)+' context</h3><p>'+esc(hub.context?.jodi?.note || '')+'</p><div class="contextTableWrap" style="margin-top:10px"><table class="contextTable"><thead><tr><th>Region</th><th>Demand</th><th>Imports</th><th>Exports</th><th>Countries</th><th>Assessed</th></tr></thead><tbody>'+jodiRows+'</tbody></table></div></div>'; document.getElementById('sourceGrid').innerHTML = sourceCards + jodiTable; }
    ...
    function openChartTab(regionKey=state.region){ window.open(chartUrl(validChartRegionKey(regionKey) ? regionKey : state.region), '_blank', 'noopener'); }
    ...
    document.getElementById('crudeRunsSheetBtn').addEventListener('click', async () => { const changed = state.sheet !== 'crude'; try { await ensureReferenceData(); } catch { showToast('Unable to load crude workbook data'); return; } state.sheet = 'crude'; if (changed) { forceCrudeRunsPeriodScroll = true; queueRender(); } refreshWorkbookSettings(); });
