...
<body>
  ...
  <script>
    ...
    document.getElementById('jsonBtn').addEventListener('click', () => downloadBlob(D.product.key + '_regional_balance_view.json','application/json',JSON.stringify({product:D.product,generatedAt:D.generatedAt,state,workbookSettings:{...workbookSettings,crudeOutages:outageEntries},chartScenarios,chartScenarioDraft,regionalBalance:D.regionalBalance,crudeRuns:D.crudeRuns,refineryCapacity:D.refineryCapacity,sourceHub:D.sourceHub,optimization:D.optimization}, null, 2)));
    document.getElementById('htmlBtn').addEventListener('click', () => downloadBlob(D.product.key + '_regional_balance_app.html','text/html','<!doctype html>\\n' + document.documentElement.outerHTML));
    document.getElementById('refreshBtn').addEventListener('click', () => { queueRender(); showToast('Dashboard refreshed'); });
