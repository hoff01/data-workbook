...
Self-contained static regional balance workbook for Distillate Fuel Oil.

Open `../Open_Diesel_Dashboard.command` on Mac or
`../Open_Diesel_Dashboard.bat` on Windows for the one-click
workflow. The launcher starts the local dashboard server in the background when
needed and opens `http://127.0.0.1:8787/Diesel_Balance/index.html`.
...
full debug bundle at `data/diesel_balance_bundle.json`.
