...
The `update:*` commands rebuild `Diesel_Balance/index.html` and `Jet_Balance/index.html` after data changes, then run the dashboard freshness check. Monthly and complete updates download the ignored `cache/eia/PET.zip` prerequisite before exporting, so a fresh GitHub clone does not require a pre-populated cache. Weekly, Monthly, Other, and Complete all attempt the full live Kpler flow package and PADD 1 split. A Kpler failure is reported as a visible **Kpler not updated** warning, preserves the existing guides and packaged PADD 1 shares, and does not block the remaining refresh or dashboard rebuild. Set `US_BALANCES_SKIP_KPLER_REFRESH=1` only when you intentionally want to skip the live attempt.
